...
function mapUpstreamError(message: string): { statusCode: number; statusMessage: string } {
  ...
  if (msg.includes('code = NotFound')) return { statusCode: 404, statusMessage: msg };
...
export default defineEventHandler(async (event) => {
  ...
  const slug = sanitizeSlug(String(article.slug || title));
  if (!title) {
  ...
  const response = await $fetch<CreatePublicationResponse>(`${capitalUrl}/query`, {
    method: 'POST',
    headers: {
    ...
    body: {
      ...
      variables: {
        publication: toPublicationInput({ ...article, title, slug }, blocks),
      },
    },
  });
  ...
  const createdSlug = String(response?.data?.createConsolePublication?.slug || '').trim();
  if (!createdSlug) {
    throw createError({
      statusCode: 502,
