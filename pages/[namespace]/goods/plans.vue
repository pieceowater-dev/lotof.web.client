...
import { getGoodsPlans, type Plan } from '@/api/goods/plans/plans';
import { subscribeToGoodsPlan, type Subscription } from '@/api/goods/plans/subscribe';
...
async function fetchPlans() {
  ...
  error.value = null;
  try {
    const result = await getGoodsPlans(nsSlug.value, false);
...
async function ensureGoodsToken(): Promise<string | null> {
  const hubToken = useCookie<string | null>('token', { path: '/' }).value;
  if (!hubToken) return null;
  const { ensure } = useGoodsToken();
  return ensure(nsSlug.value, hubToken);
}

async function fetchActiveSubscription() {
  const goodsToken = await ensureGoodsToken();
  if (!goodsToken) return;

  try {
    activeSubscription.value = await getActiveGoodsSubscription(nsSlug.value, goodsToken, 'pieceowater.goods');
    ...
    // A fetch failure is NOT the same as "confirmed no subscription" --
    // see subscriptionFetchFailed.
...
async function subscribePlan(plan: Plan) {
  ...
  if (plan.amountCents > 0 && (plan.trialDays === 0 || trialAlreadyUsed)) {
    useContactUsModal().open({ app: 'pieceowater.goods', planName: plan.name });
    return;
  }
  ...
  try {
    await subscribeToGoodsPlan(nsSlug.value, plan.code, goodsToken, 'pieceowater.goods');
    ...
    if (hubToken) {
      ...
      try {
        await hubAddAppToNamespace(hubToken, nsSlug.value, 'pieceowater.goods');
      } catch (installErr) {
    ...
    // Refresh active subscription state, then navigate into the app.
    activeSubscription.value = await getActiveGoodsSubscription(nsSlug.value, goodsToken, 'pieceowater.goods');

    useAnalytics().track('plan_subscribed', { app: 'pieceowater.goods', plan: plan.code });
...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <BillingBundlesForApp application-code="pieceowater.goods" :namespace="nsSlug" :interval="selectedInterval" />
