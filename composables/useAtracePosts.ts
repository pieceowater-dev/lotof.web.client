...
// A-Trace attendance page. `selectedPostId` and `isRouteTab` are owned by
// the page's tab-routing state and passed in so post loads can keep the
// selection in sync without this composable knowing about URL/tab concerns.
export function useAtracePosts(
  nsSlug: ComputedRef<string>,
  selectedPostId: Ref<string | null>,
  isRouteTab: ComputedRef<boolean>
) {
  ...
  // Display helpers for whichever post is currently selected
  ...
  const editForm = reactive<PostForm>(emptyPostForm());
  ...
  async function loadPosts(retryCount = 0) {
    ...
    try {
      ...
      page.value = 1;
      const res = await atracePostsList(atraceToken, nsSlug.value, { page: page.value, length: mapPaginationLength(pageLength.value) });
  ...
  async function loadMorePosts() {
    ...
    try {
      ...
      const res = await atracePostsList(atraceToken, nsSlug.value, { page: nextPage, length: mapPaginationLength(pageLength.value) });
  ...
  async function handleDelete(p: Post, _opts?: { skipConfirm?: boolean }) {
    const hubToken = useCookie<string | null>(CookieKeys.TOKEN, { path: '/' }).value;
    const atraceToken = await ensureAtraceToken(nsSlug.value, hubToken);
    if (!atraceToken) return router.push('/');
    try {
  ...
  async function handleEditSave() {
    ...
    loc.comment = norm(editForm.location.comment);
    loc.country = norm(editForm.location.country);
    loc.city = norm(editForm.location.city);
    loc.address = norm(editForm.location.address);
  ...
  return {
    ...
    selectedPost, selectedPostTitle, selectedPostLocationLine,
    page, pageLength, totalCount, loadingMore, hasMore,
