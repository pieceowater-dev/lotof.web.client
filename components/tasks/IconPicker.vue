...
import { TASK_TYPE_ICON_CHOICES } from '@/utils/taskDisplay';
...
<template>
  <UPopover v-model:open="isOpen" :popper="{ strategy: 'fixed' }">
    <button
      ...
      :title="modelValue"
    >
      <UIcon :name="modelValue || 'lucide:shapes'" class="w-5 h-5 text-gray-600 dark:text-gray-300" />
    </button>
    <template #panel>
      <div class="grid grid-cols-5 gap-1 p-2 w-56">
        <button
          v-for="icon in TASK_TYPE_ICON_CHOICES"
          :key="icon"
          type="button"
          class="flex items-center justify-center h-10 w-10 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          :class="modelValue === icon && 'bg-primary-100 dark:bg-primary-900/40 text-primary-600 dark:text-primary-300'"
          @click="pick(icon)"
        >
