      ...
      <div class="mx-auto my-8 w-full max-w-lg rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl">
        ...
        <div class="max-h-[70vh] space-y-4 overflow-y-auto p-5">
          <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
            ...
            <div>
              <div class="text-[10px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ t('admin.planForApp') }}</div>
              <div class="text-base font-bold text-slate-900 dark:text-white">{{ selectedProjectTitle }}</div>
            </div>
          </div>

          <div>
            <label for="cash-payment-namespace" class="mb-1.5 block text-xs font-semibold text-slate-700 dark:text-slate-300">{{ t('admin.namespaceSlug') }} *</label>
            <input
              id="cash-payment-namespace"
          ...
          <div>
            <label for="cash-payment-plan" class="mb-1.5 block text-xs font-semibold text-slate-700 dark:text-slate-300">{{ t('admin.plan') }} *</label>
            <select
              id="cash-payment-plan"
...
useEscapeToClose(computed(() => cashPaymentModal.value.open), () => emit('close'));
</script>
