  ...
  >
    <div class="mx-auto flex max-w-7xl flex-col items-center gap-1.5 sm:flex-row sm:justify-between">
      ...
      <div class="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs text-gray-400 dark:text-gray-500">
        <NuxtLink to="/guide" class="hover:text-gray-600 dark:hover:text-gray-300">{{ t('guide.title') || 'Гид' }}</NuxtLink>
        <NuxtLink to="/guide/global/terms" class="hover:text-gray-600 dark:hover:text-gray-300">{{ t('legal.terms') || 'Соглашение' }}</NuxtLink>
        <NuxtLink to="/guide/global/privacy" class="hover:text-gray-600 dark:hover:text-gray-300">{{ t('legal.privacy') || 'Конфиденциальность' }}</NuxtLink>
        <NuxtLink to="/guide/global/offer" class="hover:text-gray-600 dark:hover:text-gray-300">{{ t('legal.offer') || 'Оферта' }}</NuxtLink>
      </div>
    </div>
  </footer>
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div class="grid grid-cols-2 gap-8 sm:grid-cols-3 lg:grid-cols-4">
        ...
        <div>
          ...
          <ul class="mt-3 space-y-2">
            <li v-for="p in products" :key="p.to">
              <NuxtLink :to="p.to" class="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
                {{ p.label }}
              </NuxtLink>
            </li>
          </ul>
        ...
        <div>
          ...
          <ul class="mt-3 space-y-2">
            <li v-for="r in resources" :key="r.to">
              <NuxtLink :to="r.to" class="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
                {{ r.label }}
              </NuxtLink>
            </li>
          </ul>
        ...
        <div class="col-span-2 sm:col-span-1">
          ...
          <ul class="mt-3 space-y-2">
            <li>
              <NuxtLink to="/guide/global/terms" class="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
                {{ t('legal.terms') || 'Пользовательское соглашение' }}
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/guide/global/offer" class="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
                {{ t('legal.offer') || 'Публичная оферта' }}
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/guide/global/privacy" class="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
                {{ t('legal.privacy') || 'Политика конфиденциальности' }}
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/guide/global/personal-data" class="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
                {{ t('legal.personalData') || 'Обработка персональных данных' }}
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/guide/global/cookies" class="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
                {{ t('legal.cookies') || 'Файлы cookie' }}
              </NuxtLink>
            </li>
          </ul>
        </div>
      ...
      <div class="mt-10 flex flex-col items-center gap-2 border-t border-gray-100 pt-6 dark:border-gray-800 sm:flex-row sm:justify-between">
        <span class="text-xs text-gray-400 dark:text-gray-500">© {{ year }} lota — @pieceowater</span>
        <a href="mailto:pieceowater@gmail.com" class="text-xs text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">
