      ...
      <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-[520px] overflow-hidden">
        ...
        <div class="p-4 grid gap-4">
          <div v-for="group in blockGroups" :key="group.label" class="space-y-3">
            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 px-1">{{ t(`admin.editor.blockGroups.${group.label}`) }}</h4>
            <div class="grid grid-cols-3 gap-2">
              ...
              >
                ...
                <span class="text-xs text-center leading-tight font-medium text-slate-700 dark:text-slate-300">{{ t(`admin.editor.blockTypes.${item.label}`) }}</span>
              </button>
            </div>
          </div>
        </div>
