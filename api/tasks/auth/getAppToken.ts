...
export async function tasksGetAppToken(hubToken: string, namespaceSlug: string): Promise<string> {
  const devHeaders = await getDeviceHeaders();
  const res = await tasksClient.request<{ getAppToken: { token: string } }>(
    GetAppTokenDocument,
    {},
