...
export const plansApi = {
  ...
  serviceMasters: (ns: string, serviceId: string) =>
    q<{ serviceMasters: PlansMasterService[] }>(ns, `query($s:ID!){ serviceMasters(serviceId:$s){ id masterId serviceId } }`, { s: serviceId }).then(r => r.serviceMasters),
  setServiceMasters: (ns: string, serviceId: string, items: any[]) =>
    q<{ setServiceMasters: PlansMasterService[] }>(ns, `mutation($s:ID!,$i:[MasterServiceLinkInput!]!){ setServiceMasters(serviceId:$s,items:$i){ id masterId serviceId } }`, { s: serviceId, i: items }).then(r => r.setServiceMasters),
